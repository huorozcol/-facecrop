...
def save(img,name, bbox, width=180,height=227):
    ...
    #cv2.imshow('img to save', imgCrop)
    rand_num = randint(1, 1000)
    my_time = datetime.datetime.now()
    #print(my_time)
    print(f"Guardando la imagen {name}-{rand_num}.jpg")
    print(f"imgCrop: {imgCrop}")
...
def faces(frame):
    ...
    for counter,face in enumerate(faces):
        ...
        #print(f"x1-xfit:{x1-xfit} y1-yfit:{y1-yfit} x2+xfit:{x2+xfit} y2+yfit:{y2+yfit}")
